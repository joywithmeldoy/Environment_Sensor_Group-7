...
// Calculate saturation vapor pressure (hPa) at a given temperature (°C)
...
void readBME680() {
  ...
  if (SNAP.bme_ok) {
    ...
    // 1) Temperature calibration: subtract 5 °C
    float T_cal = T_raw - 5.0f;

    // 2) Correct relative humidity using saturation vapor pressure ratio
    ...
    // 3) Clamp to a valid range [0, 100] %
    if (RH_cal > 100.0f) RH_cal = 100.0f;
    if (RH_cal <   0.0f) RH_cal = 0.0f;

    // 4) Store back into SNAP (all JSON / WebSocket / OLED / SD use calibrated values)
    SNAP.tempC        = T_cal;
    ...
    float ratio = SNAP.gasR0 / R;              // The smaller R is, the more VOCs are present
    float iaq = 100.0f * powf(ratio, 1.4f);

    // Limit range (BSEC-like behavior)
...
bool setupToF() {
  // Optional: if XSHUT is connected
  if (TOF_XSHUT_PIN != -1) {
  ...
  tof.VL53L4CX_Off();
  if (tof.InitSensor(0x52) != 0) {            // 0x52 is the write-address form of 0x29
    Serial.println("[ToF] InitSensor FAIL");
    SNAP.tof_ok = false;
    return false;
  }

  // Continuous ranging (you can also set measurement budget / distance mode as needed)
...
void makeLogPath() {
  int y = 2000 + SNAP.year;   // If GPS is not fixed, year may be 0; handled below
  int m = SNAP.month;
  int d = SNAP.day;
  if (y < 2010 || m < 1 || m > 12 || d < 1 || d > 31) {
    // If GPS is not fixed, use boot time as a placeholder
...
bool setupSD() {
  ...
  // Prepare directory + list root directory
...
void loop() {
  ...
  // —— SD log: one line every 1 second ——
